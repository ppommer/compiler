...
import codegen.InputOutput;

public class Interpreter extends InputOutput {
  ...
  public static int[] parse(String textProgram) {
    ...
    // You can also use a multidimensional string array,
    // but this is cumbersome and ugly, so here is the hashtable
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      if (cmdParam.length > 1) {
        ...
        if (labels.containsKey(cmdParam[1]))
          // The parameter was defined somewhere as label
          parameter = labels.get(cmdParam[1]);
        else
          // The parameter was not defined as label, so it is a number
